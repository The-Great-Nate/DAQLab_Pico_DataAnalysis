...
def temperature_readout(target, time_create, file_type):
    ...
    with open(f"temp_data_{target}_entries_{time_create}.{file_type}", "wb") as bin_file:
        ...
        while n <= target:
                ...
                temperature_b = serial_device.read(2)
                if len(temperature_b) != 2:
                       continue
...
if __name__ == '__main__':
        ...
        if n_arg == 5:
                ...
                elapsed_time_microseconds = (end_time - start_time)
                f_csv = open(f"file_write_times.csv", "a")
                f_csv.write(f"{file_type}_struct_buffer_opt,{n},{elapsed_time_microseconds}\n")
                f_csv.close()
        elif n_arg == 2:
                ...
                elapsed_time_microseconds = (end_time - start_time)
                f_csv = open(f"file_write_times.csv", "a")
                f_csv.write(f"{file_type}_struct_buffer_opt,inf,{elapsed_time_microseconds}\n")
                f_csv.close()
        else:
